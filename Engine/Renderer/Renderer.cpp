...
namespace nu
{
    ...
    void Renderer::DrawTexture(const Texture& texture, float x, float y, float angle, float scale, bool flipH, const Vector2& origin, float offset) const
    {
        ...
        destRect.x = (x - cameraX) - (destRect.w * origin.x) - (offset * (flipH ? 1 : -1));
