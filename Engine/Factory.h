...
namespace nu
{
    ...
    template <typename T>
        requires std::derived_from<T, Object>
    inline std::unique_ptr<T> Factory::Create(const std::string& name) //this error is from not refactoring yet (I think)
    {
        std::string lowerName = ToLower(name);
        ...
        if (!m_registry.contains(lowerName))
        {
            ...
            return std::unique_ptr<T>(); //points to nothing
        }
        ...
        // check if object is derived from
