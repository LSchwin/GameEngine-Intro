...
namespace nu::json
{
    ...
    bool Read(const value_t& value, const std::string& name, int& data, bool required)
    {
        ...
        if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsInt())
        {
            ...
            if (required) std::cerr << "Could not read JSON value (int):" << name << std::endl;
            return false;
        }
    ...
    bool Read(const value_t& value, const std::string& name, bool& data, bool required)
    {
        ...
        { 
            if (required) std::cerr << "Could not read JSON value (bool):" << name << std::endl;
            return false; 
        }
    ...
    bool Read(const value_t& value, const std::string& name, std::string& data, bool required)
    {
        ...
        {
            if (required) std::cerr << "Could not read JSON value (string):" << name << std::endl;
            return false;
        }
    ...
    bool Read(const value_t& value, const std::string& name, Vector2& data, bool required)
    {
        ...
        {
            if (required) std::cerr << "Could not read JSON value (Vector2):" << name << std::endl;
            return false;
        }
